...
class CommandLog(Base):
    ...
    cmd_time = Column(DateTime(timezone=True), comment="When the command was received")
    cmd_flag = Column(Integer, server_default=text("0"), comment="Command status (CmdFlag enum)")
    cmd_exec = Column(DateTime(timezone=True), comment="When the command was executed")
