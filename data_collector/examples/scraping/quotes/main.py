"""Multi-threaded scraper: quotes.toscrape.com pages.

Demonstrates:
    - ThreadedScraper with collect() + process_batch() pattern
    - Per-thread Request via create_worker_request()
    - Thread-safe counter increments via increment_solved/increment_failed
    - Real HTTP requests to quotes.toscrape.com (practice site)
    - ORM table deployment, data storage via bulk_hash + merge
    - Structured logging with per-thread worker_id
...
Run:
    python -m data_collector.examples run scraping/quotes/main
"""
...
class Quotes(ThreadedScraper):
    """Multi-threaded scraper for quotes.toscrape.com."""
    ...
    def collect(self) -> None:
        """Distribute quote pages across worker threads."""
    ...
    def _scrape_page(self, item: Any, instance_id: int) -> None:
        """Fetch a single page and parse quotes."""
        ...
        self.logger.info("Processed page", extra={"url": item, "worker_id": instance_id, "quotes_found": len(quotes)})

    def create_worker_request(self) -> Request:
        """Create per-thread Request with custom User-Agent."""
        req = Request(timeout=30, retries=2, metrics=self.metrics)
        req.set_headers({"User-Agent": "DataCollector/1.0 (example scraper)"})
        return req
...
def _update_runtime(
    database: Database, runtime: str, start_time: datetime, scraper: Quotes | None,
) -> None:
...
def main() -> None:
    """End-to-end multi-threaded scraper example."""
