...
class TestFromZone:
    ...
    def test_different_zones_read_different_env_vars(self, monkeypatch: pytest.MonkeyPatch) -> None:
        _set_required_env(monkeypatch)
        monkeypatch.setenv("DC_PROXY_HOST_REGISTRY", "other-proxy.io")
        monkeypatch.setenv("DC_PROXY_PORT_REGISTRY", "33333")
        monkeypatch.setenv("DC_PROXY_USERNAME_REGISTRY", "brd-customer-xxx-zone-registry")
        monkeypatch.setenv("DC_PROXY_PASSWORD_REGISTRY", "other-password")

        scraping = ProxySettings.from_zone("scraping")
        registry = ProxySettings.from_zone("registry")

        assert scraping.host == "brd.superproxy.io"
        assert registry.host == "other-proxy.io"
        assert scraping.port == 22225
        assert registry.port == 33333
        assert scraping.username != registry.username
