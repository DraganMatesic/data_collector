...
4. Tracks `solved` and `failed` counts during execution (per-invocation context)
...
`self._fw_ctx.mark_solved()` / `self._fw_ctx.mark_failed()`, but `_fw_ctx` is a stable proxy that resolves to the
...
def fun_watch(func):
    def wrapper(self, *args, **kwargs):
        ...
        # self._fw_ctx is a stable proxy to that active context
        # ... execute func ...
...
If a decorated method creates child threads and worker functions call `self._fw_ctx.mark_solved()` /
`self._fw_ctx.mark_failed()`, propagate the active invocation context explicitly:

```python
registry = FunWatchRegistry.instance()

def worker() -> None:
    self._fw_ctx.mark_solved()
