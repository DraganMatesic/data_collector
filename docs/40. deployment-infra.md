...
```yaml
# File path: .github/workflows/tests-unit.yml
name: tests-unit
...
jobs:
  tests-unit:
    ...
    steps:
      ...
      - run: pytest tests/unit tests/quality --cov=data_collector --cov-report=term-missing --cov-report=xml
```

```yaml
# File path: .github/workflows/tests-integration.yml
...
- Required CI gates: `tests-unit`, `docs-quality`
- Optional/manual CI gate: `tests-integration`

### Release Workflow

```yaml
# Planned file path: .github/workflows/release.yml
name: Release
...
# Notifications (pluggable channels: Telegram, Slack, Discord, email, webhooks)
