...
```
apps/croatia/findata/company_api/
├── __init__.py    (empty)
...
```
$ python -m data_collector scaffold --group croatia --parent findata --name company_api
...
App registered in database:
  app_id:  a3f8c2...  (sha256 of croatia|fina|company_api)
  status:  NOT_RUNNING
...
Next steps:
  ...
  4. Run: python -m apps.croatia.findata.company_api.main
```
...
```python
# apps/croatia/findata/company_api/main.py
from data_collector.scraping import BaseScraper
from data_collector.utilities.hashing import bulk_hash
from apps.croatia.findata.company_api.parser import Parser
from apps.croatia.findata.company_api.tables import Companies
...
```python
# apps/croatia/findata/registry_portal/main.py
from data_collector.scraping import BaseScraper
from data_collector.utilities.hashing import bulk_hash
from apps.croatia.findata.registry_portal.parser import Parser
from apps.croatia.findata.registry_portal.tables import RegistryRecord, FetchQueue
...
```python
# apps/croatia/sudovi/court_cases/main.py
from concurrent.futures import ThreadPoolExecutor, as_completed
from data_collector.scraping import BaseScraper
from data_collector.request import Request, RequestMetrics
from data_collector.utilities.hashing import bulk_hash
from apps.croatia.sudovi.court_cases.parser import Parser
from apps.croatia.sudovi.court_cases.tables import CaseList, CaseDetail
...
```python
# apps/croatia/findata/registry_portal/main.py (bottom of file)
import argparse
import json
import uuid
...
def init(runtime, args=None):
    ...
    service = LoggingService("croatia.findata.registry_portal", db_engine=database.engine)
...
Running directly (`python -m apps.croatia.findata.registry_portal.main`) generates its own `runtime` UUID — useful for development and debugging. Pass `--args` with a JSON string for targeted runs (e.g., `--args '{"company_id":"123"}'`). Use `--help` to see available options.
...
|--------|-----------|---------|
| **Manager subprocess** | `--args` CLI flag with JSON string | `python -m apps.croatia.findata.company_api.main --args '{"company_id":"123"}'` |
| **RabbitMQ command** | Args dict in command message payload | `{"cmd": "start", "app": "...", "args": {"monitoring": true}}` |
| **Direct execution** | `argparse` CLI parsing | `python -m apps.croatia.findata.company_api.main --args '{"dry_run": true}'` |
...
```python
# apps/croatia/findata/registry_portal/parser.py
from bs4 import BeautifulSoup
from apps.croatia.findata.registry_portal.tables import RegistryRecord
