...
```
$ python -m data_collector scaffold --group croatia --parent fina --name company_api
...
Next steps:
  ...
  4. Run: python -m apps.croatia.fina.company_api.main
```
...
```python
# apps/croatia/fina/company_api/main.py
from data_collector.scraping import BaseScraper
from data_collector.utilities.hashing import bulk_hash
from apps.croatia.fina.company_api.parser import Parser
from apps.croatia.fina.company_api.tables import Companies
...
```python
# apps/croatia/fina/registry_portal/main.py
from data_collector.scraping import BaseScraper
from data_collector.utilities.hashing import bulk_hash
from apps.croatia.fina.registry_portal.parser import Parser
from apps.croatia.fina.registry_portal.tables import RegistryRecord, FetchQueue
...
```python
# apps/croatia/fina/registry_portal/main.py (bottom of file)
import argparse
import json
import uuid
...
def init(runtime, args=None):
    ...
    service = LoggingService("croatia.fina.registry_portal", db_engine=database.engine)
...
Running directly (`python -m apps.croatia.fina.registry_portal.main`) generates its own `runtime` UUID — useful for development and debugging. Pass `--args` with a JSON string for targeted runs (e.g., `--args '{"company_id":"123"}'`). Use `--help` to see available options.
...
|--------|-----------|---------|
| **Manager subprocess** | `--args` CLI flag with JSON string | `python -m apps.croatia.fina.company_api.main --args '{"company_id":"123"}'` |
| **RabbitMQ command** | Args dict in command message payload | `{"cmd": "start", "app": "...", "args": {"monitoring": true}}` |
| **Direct execution** | `argparse` CLI parsing | `python -m apps.croatia.fina.company_api.main --args '{"dry_run": true}'` |
...
```python
# apps/croatia/fina/registry_portal/parser.py
from bs4 import BeautifulSoup
from apps.croatia.fina.registry_portal.tables import RegistryRecord
