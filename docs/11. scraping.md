...
**Implementation Status:** IMPLEMENTED | **Work Package:** WP-04 | **Milestone:** v0.3.0
...
Built from the ground up based on enterprise experience with scrapers across government, financial, and legal data sources in multiple countries.
...
| **Multi-threaded** | `--type threaded` | Same + `process_batch()`, per-thread `Request` via `create_worker_request()`, shared `RequestMetrics` |
...
For high-volume collection, use `ThreadedScraper` with `process_batch()` and per-thread `Request` instances sharing a common `RequestMetrics`:

```python
# Planned file path: data_collector/country/legal/court_cases/main.py
from data_collector.scraping import ThreadedScraper
...
class CourtCases(ThreadedScraper):
    ...
    def collect(self):
        ...
        self.process_batch(self.work_list, self._scrape_case)

    def _scrape_case(self, record, instance_id):
        """Per-thread: fetch + parse + store one record."""
        request = self.create_worker_request()
        request.set_proxy(self.get_proxy())
...
> **Thread safety:** Each thread creates its own `Request` instance via `create_worker_request()` with its own HTTP session and `ExceptionDescriptor`. The shared `RequestMetrics` aggregates counters, timing, and circuit breaker data across all threads using `threading.Lock`. See [4.4. request.md — Thread Safety](4.4.%20request.md#thread-safety) for details.

## ThreadedScraper and AsyncScraper

For multi-threaded or async patterns, the framework provides `ThreadedScraper` and `AsyncScraper` variants. Both expose reusable batch processing methods (`process_batch()` / `process_batch_async()`) and leave `collect()` to the subclass. This gives full control over the collection flow while providing concurrency infrastructure.

### ThreadedScraper

`ThreadedScraper` provides `process_batch()` with `ThreadPoolExecutor`, `@fun_watch` context propagation to worker threads, and `create_worker_request()` for per-thread `Request` instances (httpx clients are not thread-safe).
...
class Quotes(ThreadedScraper):
    ...
    def _scrape_page(self, item, instance_id):
        request = self.create_worker_request()
        response = request.get(f"{self.base_url}/page/{item}")
        records = self.parser.parse(response.content)
        self.store(records)
        self.increment_solved()
```
...
`AsyncScraper` provides `process_batch_async()` with `asyncio.Semaphore` concurrency control. Uses a single shared `Request` instance (httpx `AsyncClient` is coroutine-safe). Database writes are serialized via `store_async()`.
...
class Posts(AsyncScraper):
    ...
    async def _fetch_post(self, item, instance_id):
        response = await self.request.async_get(f"{self.base_url}/posts/{item}")
        records = self.parser.parse(response.json())
        await self.store_async(records)
        self.increment_solved()
```

### process_batch() and process_batch_async()

Both methods use the same worker signature `(item: Any, index: int) -> None` and handle fatal_flag early exit. `process_batch()` additionally propagates `@fun_watch` context to worker threads when an active context exists.

**ThreadedScraper.process_batch()**
...
def process_batch(
    ...
    *,
    max_workers: int | None = None,      # defaults to self.max_workers
    track_progress: bool = True,          # calls update_progress() per item
) -> None
```

**AsyncScraper.process_batch_async()**
...
async def process_batch_async(
    ...
    *,
    max_concurrency: int | None = None,   # defaults to self.max_concurrency
    track_progress: bool = True,          # calls update_progress() per item
) -> None
```

**Example: Two-pass scraper with multiple process_batch calls**

```python
class QuotesAuthors(ThreadedScraper):
    """Collect quotes, then enrich with author biographies."""
    ...
    def _scrape_page(self, item, instance_id):
        ...
        # Parse quotes and discover author URLs
        ...

    @fun_watch
    def enrich_authors(self):
        self._fun_watch.set_task_size(len(self.author_urls))
        self.process_batch(self.author_urls, self._author_worker, track_progress=False)

    def _author_worker(self, url, index):
        request = self.create_worker_request()
...
#   Sends notification via configured channel (Telegram, Slack, etc.)
```
...
```python
def _scrape_item(self, item, instance_id):
    request = self.create_worker_request()
    url = f"{self.base_url}/{item.id}"

    # Check circuit breaker before wasting a request
    if request.is_target_unhealthy(url):
        self.increment_failed()
        return

    response = request.get(url)
    if request.should_abort(self.logger, proxy_on=True):
        return
```
