...
|
`-- data_collector/                # Framework package and application namespaces
    |-- enums/                     # Shared framework enums
    |-- settings/                  # Framework settings package
    |   |-- main.py                # Core framework settings (current)
    |   `-- country.py             # Country-specific settings (planned)
    |-- tables/
    |-- utilities/
    `-- <country>/                 # App namespace (lowercase slug)
        `-- <parent>/              # Data source namespace
            `-- <app>/             # App namespace
                |-- main.py        # Entry point
...
Planned file path: data_collector/settings/country.py (country-specific settings module).

Country namespace directories must be lowercase slugs and must not use reserved names:
`settings`, `utilities`, `tables`, `enums`, `pipeline`, `orchestration`, `api`, `notifications`, `constants`.
