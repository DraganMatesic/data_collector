...
| 4 | `thread_id` | Per OS thread | Automatic via `@fun_watch` (`threading.get_ident()`) |
...
Store the bound logger on `self`. Since `bind()` returns a new instance (no mutation), this is thread-safe. `thread_id` is auto-bound by `@fun_watch` — no manual binding needed:
...
class MyApp(Required):
    ...
    def run(self):
        ...
        with ThreadPoolExecutor(max_workers=self.no_workers) as executor:
            futures = {
                executor.submit(self.worker, record): record
    ...
    def worker(self, record):
        # thread_id is auto-bound by @fun_watch via threading.get_ident()
        self.logger.info("Processing", record_id=record.id)
        # → app_id=... runtime=... function_id=... thread_id=140234567890 record_id=42
```
...
**Source:** `data_collector/utilities/log/processors.py`
...
Finds the first non-internal frame in the call stack and populates `module_name`, `module_path`, `function_name`, and `lineno`. Skips structlog/logging/queue internals to find the actual application code that emitted the log.

`call_chain` is NOT set by this processor — it is exclusively managed by `@fun_watch` via `structlog.contextvars.bind_contextvars()`. The `merge_contextvars` processor (which runs first) pulls the bound `call_chain` into the event dict automatically.
...
| `thread_id` | `thread_id` | Auto-bound by `@fun_watch` via `threading.get_ident()` |
...
| `verify_tls` | Enables/disables TLS certificate verification (`requests.verify` bool mode) |
| `ca_bundle` | Optional CA bundle path passed to `requests.verify`; overrides `verify_tls` when provided |
| `index` | Splunk index name (default: `"default"`) |
| `sourcetype` | Splunk sourcetype (default: `"data_collector:structured"`) |
...
|----------------|-------|-------------|
| `host` | `socket.gethostname()` | Auto-detected hostname |
| `source` | `event.app_id` or logger name | Groups events by application |
| `sourcetype` | Configurable via `DC_LOG_SPLUNK_SOURCETYPE` | Enables Splunk field extraction |
| `index` | Configurable via `DC_LOG_SPLUNK_INDEX` | Routes events to specific index |
...
| `log_to_splunk` | bool | `False` | Enable Splunk HEC logging (`DC_LOG_SPLUNK_ENABLED`) |
| `splunk_hec_url` | str \| None | `None` | Splunk HEC endpoint URL (`DC_LOG_SPLUNK_URL`) |
| `splunk_token` | str \| None | `None` | Splunk HEC auth token (`DC_LOG_SPLUNK_TOKEN`) |
| `splunk_verify_tls` | bool | `True` | TLS verification mode for HEC requests (`DC_LOG_SPLUNK_VERIFY_TLS`) |
| `splunk_ca_bundle` | str \| None | `None` | Optional CA bundle path for HEC TLS verification (`DC_LOG_SPLUNK_CA_BUNDLE`) |
| `splunk_index` | str | `"default"` | Splunk index name (`DC_LOG_SPLUNK_INDEX`) |
| `splunk_sourcetype` | str | `"data_collector:structured"` | Splunk sourcetype (`DC_LOG_SPLUNK_SOURCETYPE`) |
...
Credentials for the Splunk Management API used by provisioning CLI commands.

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `mgmt_url` | str | `"https://127.0.0.1:8089"` | Splunk Management API URL (`DC_SPLUNK_MGMT_URL`) |
| `admin_user` | str \| None | `None` | Splunk admin username (`DC_SPLUNK_ADMIN_USER`) |
| `admin_password` | str \| None | `None` | Splunk admin password (`DC_SPLUNK_ADMIN_PASSWORD`) |
| `verify_tls` | bool | `False` | TLS verification for management API (`DC_SPLUNK_MGMT_VERIFY_TLS`) |

### Splunk Provisioning CLI

Create the Splunk index and sourcetype required by the framework:

```bash
python -m data_collector.tables splunk-setup
```

Empty all indexed data (keeps the index definition):

```bash
python -m data_collector.tables splunk-clean
```

Both commands read `LogSettings.splunk_index` and `LogSettings.splunk_sourcetype` for the target objects, and `SplunkAdminSettings` for Management API credentials.
...
|------|---------|---------|-----------|
| **Fixed** | `app_id`, `runtime`, `function_id`, `log_level`, `module_name`, `function_name`, `lineno`, `call_chain`, `thread_id`, `msg` | Indexed, frequently queried fields | Direct SQL/ORM filters |
