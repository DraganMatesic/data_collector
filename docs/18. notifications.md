...
**Work Package:** WP-07 | **Milestone:** v0.4.0
...
| **Discord** | Webhooks | Community and team channel alerts |
...
Severity levels are defined as `AlertSeverity` IntEnum in `data_collector/enums/notifications.py` (see [1.3. enums.md](1.3.%20enums.md)):
...
```
CRITICAL: croatia/findata/companies
...
Each channel adapter formats the notification appropriately for its platform (e.g., MarkdownV2 for Telegram, Block Kit for Slack, embeds for Discord).
...
| `notification_channels` | List | Enabled channel names (e.g., `["telegram", "slack", "email"]`) |
...
- Uses `requests` library directly for HTTP-based channels (Telegram, Slack, Discord, webhooks) and `smtplib` for email — these are simple notification POST calls, not scraping requests, so the centralized `Request` class is not used here
...
class DiscordNotifier(BaseNotifier):
    """Send alerts via Discord incoming webhook."""

    def send(self, notification: Notification) -> bool:
        embed = self.build_embed(notification)
...
To add a new channel (e.g., PagerDuty):

1. **Create the notifier class** extending `BaseNotifier`:
   ```python
   # Planned file path: data_collector/notifications/pagerduty.py
...
- **requests** — HTTP client for Telegram, Slack, Discord, and webhook channels
