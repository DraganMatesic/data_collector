...
- `FunctionLogError` table — error details for failed function invocations, cascade-deleted with `FunctionLog`
...
- Layered context binding lifecycle — app_id (static), runtime (per cycle), function_id (per function via `@fun_watch`), thread_id (per OS thread via `@fun_watch`)
...
- `LogSettings` additions: `log_format` (console/json), `log_level`, `log_error_file`, `log_error_max_bytes`, `log_error_backup_count`
...
**Notifications:**
- Pluggable notification system — Telegram, Slack, Discord, email, webhooks via channel adapters
...
- OAuth2 Client Credentials flow for service-to-service authentication (bots, automation, external integrations)
...
- **v0.5.0** — Pipeline & Integration (WP-10 Dramatiq, WP-11 WatchService, WP-12 XML, WP-13 Storage, WP-14 PDF/OCR, WP-15 Data Quality)
- **v0.6.0** — Advanced Features (WP-16 NER Pipeline)
- **v1.0.0** — Platform Release (WP-17 REST API, WP-18 Dashboard + Security + Docker)
