...
Yes. Each invocation binds its own `FunWatchContext` in context-local state. Application code still uses `self._fw_ctx.mark_solved()` / `self._fw_ctx.mark_failed()`, and `_fw_ctx` resolves to the active invocation context for the current execution flow. For child-thread fan-out inside a decorated method, wrap worker callables with `FunWatchRegistry.instance().wrap_with_active_context(...)` so workers inherit the active context and contribute to parent counters. Raw worker submissions without that wrapper have no active `@fun_watch` context and raise `RuntimeError`. Each thread writes its own `FunctionLog` row with `thread_id = threading.get_ident()`. The `Apps` table update (live dashboard progress) is performed only by the main thread to avoid race conditions. See [25. fun-watch.md](25.%20fun-watch.md).
