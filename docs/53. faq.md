...
- **Layered binding** — identifiers bound at the correct lifecycle point (app_id = static, runtime = per cycle, function_id = per function, thread_id = per OS thread via `@fun_watch`)
...
Yes. Each invocation binds its own `FunWatchContext` in context-local state. Application code still uses
`self._fun_watch.mark_solved()` / `self._fun_watch.mark_failed()`, and `_fun_watch` resolves to the active invocation context
for the current execution flow. Counter updates are synchronized, so wrapped fan-out workers can safely increment the
same parent invocation counters.

For child-thread fan-out inside a decorated method, wrap worker callables with
`FunWatchRegistry.instance().wrap_with_active_context(...)` so workers inherit the active context and can call nested
decorated methods while still returning to the parent context afterward. Raw worker submissions without that wrapper
have no active `@fun_watch` context and raise `RuntimeError`.

Each thread writes its own `FunctionLog` row with `thread_id = threading.get_ident()`. The `Apps` table update (live
dashboard progress) is performed only by the main thread to avoid race conditions. Ensure wrapped workers complete
before the decorated method returns; otherwise late updates may be missing from the logged row. See
[25. fun-watch.md](25.%20fun-watch.md).
...
**Why not Airflow?** Apps are independent (no DAG dependencies needed), and the Manager handles dynamic `next_run` scheduling based on runtime conditions — something Airflow doesn't do natively. For clients with existing Airflow infrastructure, an optional `DataCollectorOperator` can be added after the Manager (WP-09) is complete. Apps are orchestration-agnostic — same code runs under Manager or Airflow.
...
4. Global HEC input is enabled in Splunk (`[http] disabled=0`) and port `8088` is listening
5. Network allows outbound HTTPS to the Splunk server
6. `DC_LOG_SPLUNK_VERIFY_TLS` is set correctly for your certificate setup
7. If `DC_LOG_SPLUNK_VERIFY_TLS=true` and Splunk uses a private/self-signed CA, set `DC_LOG_SPLUNK_CA_BUNDLE`
8. The QueueListener is running (check for "Logging service started" in console output)
