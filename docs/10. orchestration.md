...
Apps registered through other means (example `_register_app()`, manual database inserts) default to `managed=False` and are invisible to the Manager. This allows test apps, helper scripts, and development prototypes to coexist in the same database without interfering with production orchestration.

### The `managed` Column

The `Apps` table includes a `managed` boolean column (`server_default=false`):

- **`managed=True`** -- set explicitly by `scaffold create`. The Manager schedules, monitors, and accepts commands for this app.
- **`managed=False`** -- default for all other registrations. The Manager ignores this app entirely.

The Scheduler's `get_ready_apps()` query filters on `managed == True` before checking `disable`, `run_status`, `fatal_flag`, and `next_run`.
...
# Remove from Manager oversight (managed=False, disable=True)
...
```
                 scaffold create
                       ...
                       v
    ┌─────────────────────────────────┐
    │  CREATED (managed=T, disable=T) │
    └──────────────┬──────────────────┘
                   │ scaffold enable
                   v
    ┌─────────────────────────────────┐
    │  ENABLED (managed=T, disable=F) │◄──┐
    └──────────────┬──────────────────┘   │
                   │ scaffold disable     │ scaffold enable
                   v                      │
    ┌─────────────────────────────────┐   │
    │ DISABLED (managed=T, disable=T) │───┘
    └──────────────┬──────────────────┘
                   │ scaffold unmanage
                   v
    ┌─────────────────────────────────┐
