...
**Auto-detected patterns** -- `extract_models_from_statement()` automatically detects ORM models from these statement patterns:

| Pattern | Example | Detection |
|---------|---------|-----------|
| Simple select | `select(Apps)` | `column_descriptions` |
| Column-only select | `select(Apps.app, Apps.run_status)` | `column_descriptions` |
| Aliased model | `select(aliased(Apps))` | `AliasedClass` inspection |
| Join / outerjoin | `select(A, B).join(B, ...)` | `column_descriptions` |
| `select_from` with aggregate | `select(func.count()).select_from(Apps)` | `get_final_froms()` annotations |
| Union / intersect / except | `select(A).union(select(A))` | `CompoundSelect.selects` recursion |
| Subquery in WHERE | `select(A).where(A.x.in_(subq))` | WHERE criteria recursive descent |
| EXISTS subquery | `select(A).where(exists_subq)` | WHERE criteria recursive descent |
| Cross-table WHERE | `select(A).where(A.x == B.y)` | Column annotation inspection |
| Update / Delete | `update(A).where(...)`, `delete(A)` | `entity_description` |
| Raw SQL (`text()`) | `text("SELECT 1")` | Not introspectable (empty set, no warning) |

When `map_objects` is enabled and no models are detected from a non-text statement, a warning is logged advising the developer to either pass `models=` explicitly on `run()` or restructure the query to use a database view or procedure.

#### run <a id="database-run"></a>

```python
run(self, statement: Executable, session: Session, *, models: set[type] | None = None) -> Result
```

Executes a SQLAlchemy 2.x DML statement (`update()`, `delete()`, `insert()`) with optional dependency tracking. Use this for write operations that are not covered by `merge()`, `bulk_insert()`, or `update_insert()`.

| Parameter | Type | Description |
|-----------|------|-------------|
| `statement` | `Executable` | A SQLAlchemy 2.x DML statement |
| `session` | `Session` | Active SQLAlchemy session |
| `models` | `set[type] \| None` | Explicit ORM model classes for tracking. Auto-extracted when `None`. |

```python
from sqlalchemy import update, delete
from data_collector.tables import ProxyReservation
from data_collector.utilities.database.main import Database
from data_collector.settings.main import MainDatabaseSettings
...
with database.create_session() as session:
    ...
    stmt = (
        ...
        .where(ProxyReservation.released == False)
        .values(released=True)
    )
    result = database.run(stmt, session)
    print(f"Updated {result.rowcount} rows")

    # Bulk DELETE with WHERE clause
    stmt = delete(ProxyReservation).where(ProxyReservation.released == True)
    result = database.run(stmt, session)
    session.commit()
```

**How it works internally:**
1. Auto-extracts ORM model classes from the statement (or uses explicit `models`)
2. If `app_id` is set, registers extracted models in `AppDbObjects`
3. Executes `session.execute(statement)` and returns the `Result`

#### add <a id="database-add"></a>

```python
add(self, instance: Any, session: Session, *, flush: bool = False) -> None
```

Adds a single ORM object to the session with dependency tracking. Use this instead of `session.add()` to ensure the model is registered in `AppDbObjects`.

| Parameter | Type | Description |
|-----------|------|-------------|
| `instance` | ORM object | The object to add to the session |
| `session` | `Session` | Active SQLAlchemy session |
| `flush` | `bool` | If `True`, calls `session.flush()` after add to obtain auto-generated values (e.g. PK) |

```python
from data_collector.tables import FunctionLog
from data_collector.utilities.database.main import Database
from data_collector.settings.main import MainDatabaseSettings

database = Database(MainDatabaseSettings())
with database.create_session() as session:
    record = FunctionLog(app_id="abc123", function_hash="def456")

    # Add and flush to get auto-generated ID
    database.add(record, session, flush=True)
    log_id = record.id  # Available after flush

    session.commit()
```

Caller is responsible for committing. For bulk inserts, use `bulk_insert()` instead.
...
| `run()` | Execute `update()`/`delete()`/`insert()` DML statements | N/A | Caller |
| `add()` | Add single ORM object to session with tracking | N/A | Caller |
