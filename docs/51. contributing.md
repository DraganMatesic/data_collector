...
- **IDE linter:** Pylint (VS Code extension). All rule disables go in `pyproject.toml` under `[tool.pylint."messages_control"]` — never use inline `# pylint: disable=` comments in source files
- **Type checker (CI):** mypy (strict mode)
- **Type checker (IDE):** Pyright / Pylance (strict mode in VS Code)
...
│   │   ├── test_converters.py
│   │   └── test_math.py
│   ├── request/                # Request class tests
│   │   ├── test_request_core.py
│   │   ├── test_request_async.py
│   │   ├── test_request_metrics.py
│   │   ├── test_request_soap.py
│   │   └── ...
│   └── examples/               # Examples CLI tests
│       ├── test_registry.py
...
Core dependencies listed in `requirements.txt` (httpx, zeep, sqlalchemy, pydantic, etc.) are mandatory and must always use top-level imports. Never use inline imports — Pylint enforces this.

For truly optional dependencies that are not in `requirements.txt`, use `is_module_available()` to guard imports:
