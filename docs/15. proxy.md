...
**ProxyReservation uses root domain** (e.g., `gov.hr` from `sub.gov.hr`). Reservations prevent IP collision across ALL subdomains of the same site, since target sites typically track IPs globally across subdomains. Root domain extraction uses `tldextract` (Mozilla Public Suffix List) to correctly handle TLD edge cases like `.gov.hr`, `.co.uk`, `.com.au`.

**ProxyBlacklist uses full subdomain** (e.g., `sub.gov.hr`). A ban on one subdomain does not necessarily imply a ban on other subdomains of the same domain.

`ProxyManager` handles this automatically: the constructor takes a full `target_domain` (e.g., `"sub.gov.hr"`) and extracts the root domain internally via `extract_root_domain()`.
...
The zone suffix is the uppercase app_parent name. For example, apps under `data_collector/examples/scraping/` use zone `SCRAPING`, while apps under `data_collector/croatia/sudreg/` use zone `SUDREG`. This design enables per-zone expense tracking on the BrightData dashboard.
