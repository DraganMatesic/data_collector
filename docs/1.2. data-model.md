...
Persistent configuration for watched directory roots. Each row defines a directory that `WatchService` monitors for new files. The `app_path` column links detected events to the Dramatiq actor module that processes them. Rows with `active=True` and `archive IS NULL` are loaded on startup and periodically refreshed at runtime.
...
| `app_path` | String(512) | NOT NULL | Python module path for Dramatiq actor import |
...
Inbound event record for pipeline dispatch. Any producer (WatchService, scraper, API endpoint, manual INSERT) writes an event row. The `TaskDispatcher` polls for unprocessed events and publishes Dramatiq messages to the correct exchange/queue based on `app_path`. See [17.1. dramatiq.md](17.1.%20dramatiq.md) and [17.2. watchdog.md](17.2.%20watchdog.md).

| Column | Type | Constraints | Description |
|--------|------|-------------|-------------|
| `id` | BigInteger | PK, auto-increment | Unique identifier |
| `app_path` | String(512) | NOT NULL | Python module path for dynamic import of TopicExchangeQueue definition |
| `file_path` | Text | nullable | Source document or file path |
| `document_type` | String(128) | nullable, INDEX | Document type identifier |
| `metadata_json` | Text | nullable | Arbitrary JSON payload for the event |
...
| `app_id` | String(64) | nullable, INDEX | Producer application identifier |
| `archive` | DateTime | nullable | Soft delete timestamp |
| `date_created` | DateTime | default: NOW() | Creation timestamp |
