...
```
Manager → RabbitMQ → Worker → Database
   │                    │         │
   trace_id─────────────┴─────────┘
```

Use `runtime` as the natural trace/correlation ID — it's already unique per execution and propagated through all log entries.
...
DC_LOG_SPLUNK_VERIFY_TLS=true
# Optional when using private/self-signed CA:
# DC_LOG_SPLUNK_CA_BUNDLE=/etc/ssl/certs/splunk-ca.pem
```
...
**Unauthenticated** (`GET /health`) — minimal response, no version or internal details:
```json
{
    "status": "healthy"
}
...
**Authenticated** (`GET /api/v1/system/health`) — full component breakdown:
```json
{
    "status": "healthy",
    "version": "0.5.0",
...
See [24. rest-api.md — Security Hardening](24.%20rest-api.md#response-security) for rationale on splitting authenticated vs unauthenticated health endpoints.
...
The framework uses a pluggable notification system supporting multiple channels (Telegram, Slack, Discord, email, webhooks):

| Channel | Use Case |
|---------|----------|
| **Notification system** ([18. notifications.md](18.%20notifications.md)) | Real-time alerts for fatal events, system issues (Telegram, Slack, Discord, email, webhooks) |
...
```
FATAL: scraper_company_registry
Runtime: a3f8c2d1e5b647f0912345678abcdef0
...
3. **Structure log messages** — include relevant IDs, counts, and durations
4. **Avoid logging sensitive data** — no passwords, tokens, or PII in log messages
5. **Set log rotation** — prevent disk exhaustion from verbose logging
...
- [5. logging.md](5.%20logging.md) — Logging system architecture
- [18. notifications.md](18.%20notifications.md) — Notification system (pluggable channels)
- [24. rest-api.md](24.%20rest-api.md) — Health check endpoints
