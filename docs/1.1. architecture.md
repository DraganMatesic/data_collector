...
- **Notifications** — Pluggable alert channels: Telegram Bot API, Slack API, Discord webhooks, email (SMTP), generic webhooks. Each channel implements the same notification interface
...
│
└── <country>/                           # Application namespace root (planned)
    └── <parent>/
        └── <app>/
            ├── main.py                  # App entry point
            ├── parser.py                # App-specific parsing logic
            └── tables.py                # App-specific ORM models
```

Application modules are canonically documented under `data_collector/<country>/<parent>/<app>/`.
Execution contract: `python -m data_collector.<country>.<parent>.<app>.main`.

Repository-level quality structure:
...
└── conftest.py                          # Shared test bootstrap and fixtures
```
...
| Notifications | Telegram Bot API, Slack, Discord, SMTP | latest |
