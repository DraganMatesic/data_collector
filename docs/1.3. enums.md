...
├── runtime.py           # RunStatus, FatalFlag, RuntimeExitCode, AppType
├── logging.py           # LogLevel
...
Classifies the orchestration role of an application.

```python
class AppType(IntEnum):
    STANDALONE = 0    # Manual/unmanaged app (examples, prototypes, one-off scripts)
    MANAGED = 1       # Scheduled by the orchestration Manager
    DRAMATIQ = 2      # Dramatiq worker actor (pipeline processing)
```

**Used by:** `Apps.app_type` → FK to `c_app_type`
...
|-----------|----------------|---------|-------|
| `AppType` | `c_app_type` | IntEnum | Application orchestration role |
