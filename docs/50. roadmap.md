...
| Deliverable | Description |
|-------------|-------------|
| `data_collector/scraping/base.py` | `BaseScraper` class with convention-based lifecycle: `prepare_list()`, `collect()`, `store()`, `cleanup()`, `fatal_check()` |
| `update_app_status()` | Helper function to update Apps table status during runtime |
| Progress tracking | `task_size`, `solved`, `failed`, `eta` columns on `Apps` table; `progress` as Integer (0-100) |
...
| Deliverable | Description |
|-------------|-------------|
| `data_collector/captcha/` | Provider-based architecture: `BaseCaptchaProvider` ABC + `AntiCaptchaProvider` for reCAPTCHA v2/v3, Turnstile, image CAPTCHA |
| `data_collector/settings/captcha.py` | `CaptchaSettings` with `DC_CAPTCHA_` env prefix |
...
| Implementations | Telegram, Slack, Discord, email (SMTP), webhooks |
