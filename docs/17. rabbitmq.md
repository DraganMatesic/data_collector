...
**Work Package:** WP-08 | **Milestone:** v0.4.0
...
Maps to `CmdName` IntEnum (`data_collector/enums/commands.py`, see [1.3. enums.md](1.3.%20enums.md)):
...
| Module | Description |
|--------|-------------|
| `data_collector/messaging/models.py` | `CommandMessage` frozen dataclass with JSON serialization |
| `data_collector/messaging/connection.py` | `RabbitMQConnection` with auto-reconnect and health check |
| `data_collector/messaging/topology.py` | Exchange, queue, and binding declaration functions |
| `data_collector/messaging/publisher.py` | `CommandPublisher` for publishing commands to the exchange |
| `data_collector/messaging/consumer.py` | `CommandConsumer` with background daemon thread consumption |
| `data_collector/settings/rabbitmq.py` | `RabbitMQSettings` Pydantic settings with `DC_RABBIT_` env prefix |
...
Exchange: dc_commands (direct)
    ...
    ├── Queue: manager_1_broadcast (binding: all)
    ├── Queue: manager_2 (binding: manager_2)
    └── Queue: manager_2_broadcast (binding: all)
```

- **Per-manager queues:** Commands targeted to a specific manager instance
- **Per-manager broadcast queues:** Each manager gets its own broadcast queue bound with routing key ``all`` to ensure every manager receives every broadcast command
...
| `rabbit_heartbeat` | `600` | AMQP heartbeat interval in seconds |
| `rabbit_connection_timeout` | `10` | Blocked connection timeout in seconds |
| `rabbit_reconnect_max_attempts` | `5` | Maximum reconnection attempts |
| `rabbit_reconnect_base_delay` | `1` | Base delay for exponential backoff (seconds) |
| `rabbit_reconnect_max_delay` | `30` | Maximum reconnection delay cap (seconds) |
