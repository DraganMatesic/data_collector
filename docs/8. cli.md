...
**Source:** `data_collector/examples/__main__.py`

Discovers runnable examples under `data_collector/examples/**` and executes them from the terminal.

```bash
python -m data_collector.examples <command>
```
...
| Command | Description |
|---------|-------------|
| `list [scope]` | Lists discovered runnable examples. Optional scope filters by group (for example: `request`, `database`) |
| `run <target>` | Runs one target. Supports `all`, `<scope>/all`, or exact ref `<group>/<example_name>` |

### Usage

```bash
# List all discovered runnable examples
python -m data_collector.examples list

# List only request examples
python -m data_collector.examples list request

# Run one example
python -m data_collector.examples run request/01_basic_requests

# Run all examples in a specific scope
python -m data_collector.examples run request/all

# Run all discovered examples
python -m data_collector.examples run all
```

### Prerequisites

- Many examples require internet access to reach public endpoints.
