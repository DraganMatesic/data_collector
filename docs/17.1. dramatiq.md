...
```
  ...
  table           worker_path   no dispatch    dispatch   correct exchange
...
```bash
python -m data_collector.scaffold create --group croatia --parent gazette --name ocr --type dramatiq
```
...
```python
# Planned file path: data_collector/croatia/gazette/ocr/topics.py
...
MAIN_EXCHANGE_QUEUE = TopicExchangeQueue(
    ...
    actor_path="data_collector.croatia.gazette.ocr.main",
)
```
...
───────────                    ─────                      ─────
ocr.new.gazette          →   dc_gazette_prepare    →   gazette_prepare
ocr.pdf.extract           →   dc_pdf_extract         →   process_pdf
ocr.extracted.gazette    →   dc_gazette_process    →   gazette_process
```
...
(gazette_prepare)              (process_pdf)               (gazette_process)
...
class TaskDispatcher:
    ...
    def _dispatch_event(self, event, session) -> bool:
        ...
        module = importlib.import_module(event.worker_path)
...
@dramatiq.actor(queue_name=EOGLASNA_PREPARE.name, max_retries=3,
                on_retry_exhausted="log_dead_letter")
def gazette_prepare(event_id: int):
    ...
    with database.create_session() as session:
        ...
        existing = session.execute(
            ...
            .where(EventProcessingStatus.event_id == event_id,
                   EventProcessingStatus.actor_name == "gazette_prepare")
        ).scalar_one_or_none()
        if existing:
            return
...
@dramatiq.actor(queue_name=EOGLASNA_PROCESS.name, max_retries=3,
                on_retry_exhausted="log_dead_letter")
def gazette_process(event_id: int):
...
dc_gazette_prepare         → log_dead_letter actor
dc_gazette_process         → log_dead_letter actor
```
...
DC_DRAMATIQ_QUEUES=dc_pdf_extract,dc_gazette_prepare dramatiq data_collector.dramatiq.actors
```
...
set DC_DRAMATIQ_QUEUES=dc_pdf_extract,dc_gazette_prepare
