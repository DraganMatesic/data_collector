...
EOGLASNA_PREPARE = TopicExchangeQueue(
    name="dc_gazette_prepare",
    actor_name="gazette_prepare",
    exchange_name=OCR_TOPIC_EXCHANGE.name,
    routing_key="ocr.new.gazette",
    actor_path="apps.croatia.gazette.workers.ocr"
)

EOGLASNA_PROCESS = TopicExchangeQueue(
    name="dc_gazette_process",
    actor_name="gazette_process",
    exchange_name=OCR_TOPIC_EXCHANGE.name,
    routing_key="ocr.extracted.gazette",
    actor_path="apps.croatia.gazette.workers.ocr"
)
```
...
```python
# data_collector/pipeline/task_dispatcher/main.py
...
```python
# apps/croatia/gazette/workers/ocr.py
...
**PipelineStatus** and **PipelineStage** are defined as IntEnum in `data_collector/enums/pipeline.py` (see [1.3. enums.md](1.3.%20enums.md)).
