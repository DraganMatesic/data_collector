...
| `splunk_token` | str \| None | `None` | Splunk HEC authentication token (`DC_LOG_SPLUNK_TOKEN`) |
| `splunk_verify_tls` | bool | `True` | Verify Splunk HEC TLS certificate (`DC_LOG_SPLUNK_VERIFY_TLS`) |
| `splunk_ca_bundle` | str \| None | `None` | Optional CA bundle path for Splunk HEC TLS (`DC_LOG_SPLUNK_CA_BUNDLE`) |
...
| `log_error_file` | str | `"error.log"` | Path for sink failure fallback log (`DC_LOG_ERROR_FILE`) |
...
| `DC_LOG_SPLUNK_ENABLED` | LogSettings | No | Enable Splunk HEC sink |
| `DC_LOG_SPLUNK_URL` | LogSettings | No | Splunk HEC base endpoint (for example `https://127.0.0.1:8088/services/collector`) |
| `DC_LOG_SPLUNK_TOKEN` | LogSettings | No | Splunk HEC token |
| `DC_LOG_SPLUNK_VERIFY_TLS` | LogSettings | No | TLS verification flag for Splunk HEC requests |
| `DC_LOG_SPLUNK_CA_BUNDLE` | LogSettings | No | CA bundle path for Splunk HEC TLS verification |
| `DC_LOG_ERROR_FILE` | LogSettings | No | Override fallback sink error file path (empty value keeps default `error.log`) |
| `DC_SECRET_PASSWORD` | SecretLoader | No | Password for decrypting secrets.env.enc |

## NotificationSettings <a id="notification-settings"></a>
...
| Discord | Available | Webhook-based notifications |
